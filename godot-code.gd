...
var drag_accum_y = 0.0
const SWIPE_THRESHOLD = 30
const DRAG_SENSITIVITY = 35 
...
func build_pause_screen():
	...
	restart_btn.position = Vector2(200, 500)
	restart_btn.pressed.connect(_on_restart_pressed)
	pause_ui_layer.add_child(restart_btn)
...
func toggle_pause():
	if game_state == "PLAYING":
		...
		$FallTimer.stop()
		pause_ui_layer.show()
		if has_node("BGM"): $BGM.stream_paused = true
	elif game_state == "PAUSED":
		game_state = "PLAYING"
		$FallTimer.start()
		pause_ui_layer.hide()
		if has_node("BGM"): $BGM.stream_paused = false
...
func spawn_piece(is_first_piece = false):
	var shape_idx
	
	if is_first_piece:
		shape_idx = [0, 1].pick_random() 
		next_queue.pop_front() 
	else:
		shape_idx = next_queue.pop_front()
...
func _input(event):
	...
	if event is InputEventScreenTouch:
		if event.pressed:
			...
			var swipe_time = Time.get_ticks_msec() - touch_start_time
			var swipe_vector = event.position - touch_start_pos
			
			if swipe_vector.y > 60 and abs(swipe_vector.y) > abs(swipe_vector.x) and swipe_time < 300:
				hard_drop()
			elif not has_dragged and swipe_vector.length() < 15:
		...
		drag_accum_y += event.relative.y

		if drag_accum_x > DRAG_SENSITIVITY:
			if valid_move(1, 0):
				piece_pos.x += 1
				has_dragged = true
				last_action = "move"
		...
		elif drag_accum_x < -DRAG_SENSITIVITY:
			if valid_move(-1, 0):
				piece_pos.x -= 1
				has_dragged = true
				last_action = "move"
		...
		if drag_accum_y > DRAG_SENSITIVITY:
